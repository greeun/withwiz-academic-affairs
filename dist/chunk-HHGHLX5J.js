...
function useAdminList(options) {
  ...
  const fetchList = _react.useCallback.call(void 0, 
    async (sort) => {
      try {
        const res = await _chunkRKWWRCLNjs.adminFetch.call(void 0, `${apiPath}?limit=100&sortBy=${sort}`);
        if (res.ok) {
...
async function uploadSingleFile(file) {
  ...
  fd.append("file", file);
  const res = await _chunkRKWWRCLNjs.adminFetch.call(void 0, "/api/admin/upload", {
    method: "POST",
    body: fd
  });
  const json = await res.json();
...
//# sourceMappingURL=chunk-HHGHLX5J.js.map