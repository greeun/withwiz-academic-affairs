...
function AdminManagerBaseInner({
  ...
  const list = _chunkGVEUBO56js.useAdminList.call(void 0, {
    initialItems,
  ...
  const adminForm = _chunkGVEUBO56js.useAdminForm.call(void 0, config.emptyForm);
  const {
    tab,
    setTab,
    form,
  ...
  const selectItem = _react.useCallback.call(void 0, async (id) => {
    ...
    try {
      const res = await _chunkFJCSRKGNjs.adminFetch.call(void 0, `${config.apiPath}/${id}`);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await _chunkFJCSRKGNjs.adminFetch.call(void 0, url, {
        method,
      ...
      const json = await res.json();
      if (json.success) {
        const listRes = await _chunkFJCSRKGNjs.adminFetch.call(void 0, `${config.apiPath}?limit=100&sortBy=${sortKey}`);
...
function ImageDropUpload({
  ...
  const drop = _chunkGVEUBO56js.useImageDropZone.call(void 0, {
...
//# sourceMappingURL=chunk-MWQINSMZ.js.map